...
# Ajustar el modelo Ridge (regresión polinómica con regularización)
model = Ridge(alpha=0.1)  # Ajustar el parámetro alpha para controlar la regularización
model.fit(X_poly, y)
...
plt.plot(df_decada['decade'], model.predict(X_poly), color='green', label='Ajuste Polinómico (Ridge)', linestyle='-')
plt.plot(future_decades, forecast, label='Predicción Polinómica (Ridge)', linestyle='--', color='red', marker='x')
plt.title('Predicción Suavizada de la Temperatura Promedio del Mar Mediterráneo')
plt.xlabel('Década')
